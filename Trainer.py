...
# MARK: building model
# building model will create a set of tensorflow variables
# each tensorflow variable is a node in computation graph
# tensorflow variables do not have any value before execution
...
# MARK: computation graph is created. Until now, all node contain no value at all, only
# how they will execute, IF they were given any input
with tf.Session() as sess:
    sess.run(init)  # runs the "init" node, which initializes all variables in ways
    # defined above, when creating the graph
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            # reads a batch of input data. Everything in numpy (python), not in tensorflow
            _, c = sess.run([optimizer, cost], feed_dict={x_batch: x, emb_mask: batch_mask, word_num: word_number, y_batch: y})
            # executes only the nodes necessary to compute both optimizer and cost, with placeholder values given by
            # feed_dict.
